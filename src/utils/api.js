...
export const removeMessage = async (messageId, token) => {
  try {
    // Send a DELETE request to the API endpoint for removing a message
    const res = await apiRequest({
...
export const createGroupChat = async (members, groupename, token) => {
  ...
  //export const createChat = (data, token) => apiRequest({ url: '/chat/create', data, method: 'POST', token });
...
// Define the getUsersForSidebar function
export const getUsersForSidebar = async (token, query) => {
  try {
    const response = await apiRequest({
      url: "/chat/search",
      token: token,
...
export const getUsersBySearch = async (firstName, lastName, token) => {
  try {
    const response = await apiRequest({
      url: `/users/searchUsers?firstName=${firstName}&lastName=${lastName}`,
      token: token,
      method: "GET",
    });
    return response.data;
